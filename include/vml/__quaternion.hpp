...
inline constexpr vector_options __mtl_quaternion_vector_options =
...
public:
    ...
    constexpr quaternion(T const& real, _VMTL::vector<T, 3, P> const& imag):
        __mtl_base(real, imag.__mtl_at(0), imag.__mtl_at(1), imag.__mtl_at(2)) {
    }
    constexpr quaternion(T const& a, T const& b, T const& c, T const& d):
        __mtl_base(a, b, c, d) {}
    template <std::convertible_to<T> U>
    constexpr quaternion(complex<U> const& z):
        quaternion(z.__mtl_at(0), z.__mtl_at(1), 0, 0) {}
    template <vector_options P = __mtl_quaternion_vector_options>
...
__mtl_mathfunction __mtl_always_inline __mtl_interface_export constexpr vector<
    T, 3, __mtl_quaternion_vector_options>
    imag(quaternion<T> const& z) {
    return { z.__mtl_at(1), z.__mtl_at(2), z.__mtl_at(3) };
}

template <typename T>
__mtl_mathfunction __mtl_always_inline constexpr vector<
    T, 4, __mtl_quaternion_vector_options>&
    __mtl_as_vector(quaternion<T>& z) {
    return static_cast<vector<T, 4, quaternion<T>::__mtl_options>&>(z);
}
template <typename T>
__mtl_mathfunction __mtl_always_inline constexpr vector<
    T, 4, __mtl_quaternion_vector_options> const&
    __mtl_as_vector(quaternion<T> const& z) {
...
constexpr bool isnan(quaternion<T> const& z) {
    ...
    return isnan(z.__mtl_at(0)) || isnan(z.__mtl_at(1)) ||
           isnan(z.__mtl_at(2)) || isnan(z.__mtl_at(3));
}
...
constexpr bool isinf(quaternion<T> const& z) {
    ...
    return isinf(z.__mtl_at(0)) || isinf(z.__mtl_at(1)) ||
           isinf(z.__mtl_at(2)) || isinf(z.__mtl_at(3));
}
...
constexpr bool isfinite(quaternion<T> const& z) {
    ...
    return isfinite(z.__mtl_at(0)) || isfinite(z.__mtl_at(1)) ||
           isfinite(z.__mtl_at(2)) || isfinite(z.__mtl_at(3));
}
...
constexpr bool isnormal(quaternion<T> const& z) {
    ...
    return isnormal(z.__mtl_at(0)) || isnormal(z.__mtl_at(1)) ||
           isnormal(z.__mtl_at(2)) || isnormal(z.__mtl_at(3));
}
...
/*
 ...
 these 'using namespace mtl::quaternion_literals;' also pulls in the _i* complex
 literals
 */
...
__mtl_mathfunction __mtl_always_inline
    __mtl_interface_export constexpr quaternion<T>
...
__mtl_mathfunction __mtl_always_inline
    __mtl_interface_export constexpr quaternion<__mtl_promote(T, U)>
...
__mtl_mathfunction __mtl_always_inline
    ...
    return quaternion<__mtl_promote(T, U)>(real(a) + real(b),
                                           imag(b) + a.__mtl_at(1),
                                           a.__mtl_at(2), a.__mtl_at(3));
}
...
__mtl_mathfunction __mtl_always_inline
    ...
    return quaternion<__mtl_promote(T, U)>(real(a) + b, a.__mtl_at(1),
                                           a.__mtl_at(2), a.__mtl_at(3));
}
...
__mtl_mathfunction __mtl_always_inline
    ...
    return quaternion<__mtl_promote(T, U)>(a + real(b), b.__mtl_at(1),
                                           b.__mtl_at(2), b.__mtl_at(3));
}
...
__mtl_mathfunction __mtl_always_inline
    ...
    return quaternion<__mtl_promote(T, U)>(
        a.__mtl_at(0) * b.__mtl_at(0) - a.__mtl_at(1) * b.__mtl_at(1) -
            a.__mtl_at(2) * b.__mtl_at(2) - a.__mtl_at(3) * b.__mtl_at(3), // 1
        a.__mtl_at(0) * b.__mtl_at(1) + a.__mtl_at(1) * b.__mtl_at(0) +
            a.__mtl_at(2) * b.__mtl_at(3) - a.__mtl_at(3) * b.__mtl_at(2), // i
        a.__mtl_at(0) * b.__mtl_at(2) - a.__mtl_at(1) * b.__mtl_at(3) +
            a.__mtl_at(2) * b.__mtl_at(0) + a.__mtl_at(3) * b.__mtl_at(1), // j
        a.__mtl_at(0) * b.__mtl_at(3) + a.__mtl_at(1) * b.__mtl_at(2) -
...
__mtl_mathfunction __mtl_always_inline __mtl_interface_export constexpr bool
    operator==(quaternion<T> const& a, complex<U> const& b) {
    return real(a) == real(b) && a.__mtl_at(1) == imag(b) &&
           a.__mtl_at(2) == 0 && a.__mtl_at(3) == 0;
}
...
__mtl_mathfunction __mtl_always_inline __mtl_interface_export constexpr bool
    operator==(quaternion<T> const& a, U const& b) {
    return real(a) == b && a.__mtl_at(1) == 0 && a.__mtl_at(2) == 0 &&
           a.__mtl_at(3) == 0;
}
...
__mtl_mathfunction __mtl_always_inline __mtl_interface_export constexpr bool
    operator==(complex<T> const& a, quaternion<U> const& b) {
    return real(a) == real(b) && imag(a) == b.__mtl_at(1) &&
           0 == b.__mtl_at(2) && 0 == b.__mtl_at(3);
}
...
__mtl_mathfunction __mtl_always_inline __mtl_interface_export constexpr bool
    operator==(T const& a, quaternion<U> const& b) {
    return a == real(b) && 0 == b.__mtl_at(1) && 0 == b.__mtl_at(2) &&
           0 == b.__mtl_at(3);
}
...
template <real_scalar T, real_scalar U>
__mtl_mathfunction __mtl_always_inline
    __mtl_interface_export constexpr quaternion<
...
quaternion<__mtl_floatify(T)> sinh(quaternion<T> const& _z) {
    ...
    __mtl_safe_math_if(real(z) == 0 &&
                       !std::isinf(v_norm)) return complex<F>(real(z), F(NAN));
    __mtl_safe_math_if(v_norm == 0 && !std::isinf(real(z))) return z;
...
quaternion<__mtl_floatify(T)> cosh(quaternion<T> const& _z) {
    ...
    quaternion<F> z = _z;
    auto const v_norm = _VMTL::norm(z.vector);
    __mtl_safe_math_if(std::isinf(real(z)) && !std::isinf(v_norm)) return {
        std::abs(real(z)), F(NAN), F(NAN), F(NAN)
    };
    __mtl_safe_math_if(real(z) == 0 && !std::isinf(v_norm)) return { F(NAN), 0,
                                                                     0, 0 };
    __mtl_safe_math_if(real(z) == 0 && v_norm == 0) return { F(1), 0, 0, 0 };
    __mtl_safe_math_if(v_norm == 0 && !std::isinf(real(z))) return {
...
constexpr quaternion<__mtl_floatify(__mtl_promote(T, U))> make_rotation(
...
constexpr vector3<__mtl_floatify(__mtl_promote(T, U)), O> rotate(
...
template <typename T>
struct std::tuple_size<_VMTL::quaternion<T>>:
